...
public class VelocityOABungeecordListener extends VelocityBungeecordListener {

	public VelocityOABungeecordListener(@NotNull ADPPlugin plugin) {
	...
	protected void handlePacket(byte[] bytes, @NotNull MessageChannel messageChannel) {
		...
		if (packet != null) {
			...
			if (VelocityConfigMain.BLOCKS_LISTALLOWED.contains("*") || VelocityConfigMain.BLOCKS_LISTALLOWED.contains(packet.getData().getBlock())) {
